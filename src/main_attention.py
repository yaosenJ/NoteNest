...
log_file = "/root/kg-baseline-pytorch-master/kg-baseline-pytorch-master/coal/training_log_batch_size_16_word_emb_size_128_epoch_500_attention.txt"
...
# loss = torch.nn.CrossEntropyLoss().cuda() #张量 input 和目标张量 target 的维数 不需要一样
# b_loss = torch.nn.BCEWithLogitsLoss().cuda() #二值交叉熵损失函数 传入的两个张量的维度需要相同
...
for i in range(EPOCH_NUM):
    ...
    if f1 >= best_f1:
        best_f1 = f1
        best_epoch = i
        torch.save(s_m, r'/root/kg-baseline-pytorch-master/kg-baseline-pytorch-master/coal/attention_models/s_'+str(i)+'.pkl')
        torch.save(po_m, '/root/kg-baseline-pytorch-master/kg-baseline-pytorch-master/coal/attention_models/po_'+str(i)+'.pkl')
